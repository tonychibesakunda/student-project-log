...
$app->post('/student/schedules/add_scheduled_meeting', $student(), function() use($app){
	...
	if(isset($_POST['add'])){
		...
		$v->addRule('uniqueScheduledDate', function($value, $input, $args) use($app){
			...
			if($select_supervisor > 0){
				return $select_supervisor;
			}

			//get supervision
			$get_spid = "SELECT supervisor_id FROM supervisions WHERE student_id=$student_id AND supervisor_id=$select_supervisor";
			$sp_id = DB::select(DB::raw($get_spid));

			foreach ($sp_id as $sp) {
				$sp_id = $sp->supervisor_id;
			}

		    $po = DB::table('scheduled_meeetings')
